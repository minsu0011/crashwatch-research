...
def main() -> int:
    parser = argparse.ArgumentParser(description="Supervise CrashWatch temporal breadth game-mode validation")
    ...
    command = [
        sys.executable,
        str(package / "run_regime_temporal_breadth.py"),
        "--selection-output", str(Path(args.selection_output).expanduser().resolve()),
        "--prior-followup", str(Path(args.prior_followup).expanduser().resolve()),
        "--output", str(output),
    ...
    for attempt in range(1, max(1, int(args.max_restarts) + 1) + 1):
        if (output / "STOP_TEMPORAL_BREADTH").exists():
            return 3
        started = time.time()
        print(f"TEMPORAL_SUPERVISOR attempt={attempt} command={command}", flush=True)
        completed = subprocess.run(command, cwd=package, env=os.environ.copy(), check=False)
        final = _read(output / "FINAL_TEMPORAL_BREADTH_STATUS.json")
        ok = bool(
        ...
        (output / "TEMPORAL_SUPERVISOR_ATTEMPTS.json").write_text(json.dumps(attempts, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        if ok:
            print(json.dumps({"status": "completed", "final": final}, ensure_ascii=False), flush=True)
            return 0
        budget = _read(output / "TEMPORAL_BUDGET.json")
