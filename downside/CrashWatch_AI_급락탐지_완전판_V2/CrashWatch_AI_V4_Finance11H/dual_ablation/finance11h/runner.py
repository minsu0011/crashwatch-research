...
class TaskResult:
    ...
    fallback_used: bool
    oom_detected: bool
    fit_attempts_json: str
    cache_validation_error: str = ""
...
def _summarize_deltas(delta: pd.DataFrame) -> pd.DataFrame:
    ...
    metric_cols = [
        "pr_auc_loss", "roc_auc_loss", "balanced_accuracy_loss", "accuracy_loss",
        "brier_increase", "logloss_increase",
        "top_1pct_precision_loss", "top_1pct_recall_loss",
        "top_3pct_precision_loss", "top_3pct_recall_loss",
        "top_5pct_precision_loss", "top_5pct_recall_loss",
    ]
    for key_values, block in delta.groupby(keys, dropna=False, sort=False):
        ...
        row["inference_level"] = (
            "statistical" if row["fold_count"] >= 8
            else "directional" if row["fold_count"] >= 4
            else "exploratory"
        )
        for col in metric_cols:
            ...
            row[f"{col}_sign_p"] = _exact_sign_p(values) if len(values) >= 8 else np.nan
        rows.append(row)
    return pd.DataFrame(rows)


def _pair_ablation_metrics(metrics: pd.DataFrame) -> pd.DataFrame:
    baseline_metrics = metrics.loc[metrics["experiment"].eq("baseline")].copy()
    work = metrics.copy()
    work["pair_scope_type"] = work["scope_type"].replace({"target_bucket": "bucket", "target_ticker": "ticker"})
    key = ["fold", "seed", "pair_scope_type", "scope_value"]
    base_cols = [
        "pr_auc", "roc_auc", "balanced_accuracy", "accuracy", "brier", "logloss",
        "top_1pct_precision", "top_1pct_recall",
        "top_3pct_precision", "top_3pct_recall",
    ...
    compared = work.loc[~work["experiment"].isin(["baseline", "tune_baseline"])].merge(
    ...
    for fraction in (1, 3, 5):
        for metric in ("precision", "recall"):
            column = f"top_{fraction}pct_{metric}"
            compared[f"{column}_loss"] = compared[f"baseline_{column}"] - compared[column]
    return compared


def _build_sensitivity_matrices(summary: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    bucket = (
        bucket_source.pivot_table(index="target_bucket", columns="group", values="pr_auc_loss_mean", aggfunc="mean").reset_index()
        if not bucket_source.empty else pd.DataFrame()
    )
    ...
    ticker = (
        ticker_source.pivot_table(index="target_ticker", columns="group", values="pr_auc_loss_mean", aggfunc="mean").reset_index()
        if not ticker_source.empty else pd.DataFrame()
    )
...
class Finance11HRunner:
    def __init__(
        ...
        *,
        ...
        cache_namespace: str = "finance11h_v1",
    ) -> None:
        ...
        self.task_times_by_stage: dict[str, list[float]] = {}
    ...
    def can_start(self, stage: str | None = None) -> bool:
        if self.monitor.abort_event.is_set():
            return False
        stage_times = self.task_times_by_stage.get(stage or "", [])
        estimate_source = stage_times[-20:] if stage_times else self.task_times[-20:]
        estimate = np.median(estimate_source) if estimate_source else 180.0
    ...
    def load(self) -> None:
        ...
        target = pd.to_numeric(df["label_abs_crash_20"], errors="coerce")
        if target.isna().any():
            raise RuntimeError(f"target 결측을 음성으로 치환할 수 없습니다: missing={int(target.isna().sum())}")
        if not target.isin([0, 1]).all():
            raise RuntimeError("label_abs_crash_20은 0/1 이외 값을 포함합니다.")
        if "sealed_do_not_train_or_tune" in df.columns:
            sealed = pd.to_numeric(df["sealed_do_not_train_or_tune"], errors="coerce").fillna(0)
            if sealed.ne(0).any():
                raise RuntimeError(f"development에 sealed 행이 포함되어 있습니다: {int(sealed.ne(0).sum())}")
        ...
        mandatory = {
            "t_financial_shorting": 8,
            "u_financial_shorting": 8,
            "t_financial_flow": 1,
            "u_financial_flow": 1,
            "t_financial_valuation": 1,
            "t_financial_interaction": 1,
        }
        insufficient = {
            group: {"valid": len(self.catalog.get(group, [])), "required": minimum}
            for group, minimum in mandatory.items()
            if len(self.catalog.get(group, [])) < minimum
        }
        if insufficient:
            raise RuntimeError(f"필수 금융 그룹 유효 피처 부족: {insufficient}")
        ...
        self.y = pd.to_numeric(self.df["label_abs_crash_20"], errors="raise").astype(np.int8).to_numpy()
    ...
    def _validate_cached_prediction(self, pred: pd.DataFrame, val_idx: np.ndarray) -> str:
        required = {"date", "ticker", "bucket", "target", "prediction"}
        missing = sorted(required - set(pred.columns))
        if missing:
            return f"missing_columns={missing}"
        expected = self.df.loc[val_idx, ["date", "ticker", "bucket"]].sort_values(["date", "ticker"]).reset_index(drop=True)
        actual = normalize_date(pred[["date", "ticker", "bucket"]]).sort_values(["date", "ticker"]).reset_index(drop=True)
        actual["ticker"] = normalize_ticker(actual["ticker"])
        actual["bucket"] = actual["bucket"].astype(str)
        expected["ticker"] = normalize_ticker(expected["ticker"])
        expected["bucket"] = expected["bucket"].astype(str)
        if len(actual) != len(expected):
            return f"row_count={len(actual)} expected={len(expected)}"
        if not actual.equals(expected):
            return "validation_row_identity_mismatch"
        pred_target = pred.assign(
            ticker=normalize_ticker(pred["ticker"])
        ).sort_values(["date", "ticker"])["target"]
        expected_target = self.df.loc[val_idx].assign(
            _target=self.y[val_idx]
        ).sort_values(["date", "ticker"])["_target"]
        if not np.array_equal(pd.to_numeric(pred_target, errors="coerce").to_numpy(), expected_target.to_numpy()):
    ...
    def execute(self, exp: Experiment, fold: dict, seed: int, config: ModelConfig) -> tuple[pd.DataFrame, TaskResult]:
        ...
        fit_diagnostics: dict = {"fallback": False, "oom_detected": False, "attempts": []}
        if cache.exists() and not self.overwrite_cache:
            pred = pd.read_parquet(cache)
            cache_validation_error = self._validate_cached_prediction(pred, val_idx)
            if not cache_validation_error:
                backend = str(pred["_model_backend"].iloc[0]) if "_model_backend" in pred and len(pred) else "cache_unknown"
                if "_fit_diagnostics_json" in pred and len(pred):
                    try:
                        fit_diagnostics = json.loads(str(pred["_fit_diagnostics_json"].iloc[0]))
                    except json.JSONDecodeError:
                        fit_diagnostics = {"fallback": backend != "xgboost_cuda", "oom_detected": False, "attempts": []}
                status = "hit"
            else:
                LOGGER.warning("캐시 무효화 %s: %s", cache, cache_validation_error)
                status = "invalidated"
        else:
            status = "miss"
        if status != "hit":
            model, backend, fit_diagnostics = fit_xgb(
                x_train, self.y[train_idx], seed, config,
                threads=self.threads, prefer_gpu=self.prefer_gpu,
            )
            ...
            pred["_model_backend"] = backend
            pred["_fit_diagnostics_json"] = json.dumps(fit_diagnostics, ensure_ascii=False, sort_keys=True)
            atomic_parquet(pred, cache)
            if status == "invalidated":
                status = "invalidated_miss"
        elapsed = time.perf_counter() - started
        if status != "hit":
            self.task_times.append(elapsed)
            self.task_times_by_stage.setdefault(exp.stage, []).append(elapsed)
        ...
        result = TaskResult(
            ...
            fallback_used=bool(fit_diagnostics.get("fallback", False)),
            oom_detected=bool(fit_diagnostics.get("oom_detected", False)),
            fit_attempts_json=json.dumps(fit_diagnostics.get("attempts", []), ensure_ascii=False, sort_keys=True),
    ...
    def tune(self) -> ModelConfig:
        ...
        comparable = summary.loc[summary["completed_folds"].ge(len(tune_folds))].copy()
    ...
    def _run_batch(self, experiments: list[Experiment], fold_ids: list[int], seeds: list[int], config: ModelConfig, metrics_frames: list[pd.DataFrame], records: list[dict]) -> None:
        for exp in experiments:
            ...
            for fold_id in fold_ids:
                ...
                for seed in seeds:
                    if not self.can_start(exp.stage):
                        return
                    ...
                    try:
                        ...
                        records.append({
                            **asdict(exp), "fold": fold_id, "seed": seed, "status": "failed",
                        ...
                        atomic_json({
                            ...
                            "remaining_hours": self.remaining()/3600,
                            "completed_tasks": len([r for r in records if r.get("status") == "completed"]),
                            "failed_tasks": len([r for r in records if r.get("status") == "failed"]),
                            "resource_monitor": self.monitor.status(),
        ...
        try:
            ...
            subclusters = {
                "t_finshort_volume_cluster": [
                    ...
                    if ("volume" in c or "value" in c) and "balance" not in c
                ],
            ...
            bucket_groups = [
                g for g in [
                    "t_financial_shorting", "t_finshort_crowding_cluster",
                    "t_financial_interaction", "t_financial_flow", "t_financial_valuation",
                ]
            ...
            baseline_metrics = metrics.loc[metrics["experiment"].eq("baseline")].copy()
            compared = _pair_ablation_metrics(metrics)
            ...
            bucket_matrix, ticker_matrix = _build_sensitivity_matrices(summary)
            if not bucket_matrix.empty:
                atomic_csv(bucket_matrix, self.result_dir / "bucket_finance_sensitivity_matrix.csv")
            if not ticker_matrix.empty:
                atomic_csv(ticker_matrix, self.result_dir / "ticker_finance_sensitivity_matrix.csv")
            ...
            goal = {
                ...
                "mean_top_1pct_precision": means.get("top_1pct_precision"),
                "mean_top_1pct_recall": means.get("top_1pct_recall"),
                "mean_top_3pct_precision": means.get("top_3pct_precision"),
                "mean_top_3pct_recall": means.get("top_3pct_recall"),
                "mean_top_5pct_precision": means.get("top_5pct_precision"),
                "mean_top_5pct_recall": means.get("top_5pct_recall"),
            ...
            result = {
                ...
                "new_models": int(sum(r.get("cache_status") in {"miss", "invalidated_miss"} for r in records)),
                "failed_tasks": int(sum(r.get("status") == "failed" for r in records)),
                "gpu_fallback_tasks": int(sum(bool(r.get("fallback_used")) for r in records)),
                "gpu_oom_tasks": int(sum(bool(r.get("oom_detected")) for r in records)),
