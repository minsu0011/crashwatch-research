...
def rolling_corr(a: np.ndarray, b: np.ndarray, window: int, min_periods: int) -> np.ndarray:
    ...
    for i in range(n):
        ...
        if i >= window:
            old_a = a[i - window]
            old_b = b[i - window]
            if _finite(old_a) and _finite(old_b):
                count -= 1
                sa -= old_a
                sb -= old_b
                saa -= old_a * old_a
...
def _exercise_kernels() -> None:
    ...
    matrix = np.ascontiguousarray(np.column_stack([x, y]), dtype=np.float64)
    rowwise_weighted_mean(matrix, np.ones((256, 2), dtype=np.float64))


def numba_diagnostics() -> dict:
    kernels = {
        ...
        "rowwise_weighted_mean": rowwise_weighted_mean,
    }
    signatures = {
        name: [str(signature) for signature in getattr(kernel, "nopython_signatures", [])]
        for name, kernel in kernels.items()
    }
    return {
        "numba_available": NUMBA_AVAILABLE,
        "nopython_compiled": bool(NUMBA_AVAILABLE and all(signatures.values())),
        "nopython_signatures": signatures,
    }


def warmup() -> dict:
    """Compile all kernels and separately measure warm and repeated execution."""
    started = time.perf_counter()
    _exercise_kernels()
    first_dispatch_seconds = time.perf_counter() - started
    started = time.perf_counter()
    _exercise_kernels()
    repeated_seconds = time.perf_counter() - started
    return {
        "first_dispatch_seconds": first_dispatch_seconds,
