...
ENDPOINT_PATH_TOKENS = {
    "getStckLnbDetail": "d",
    "getStckLnbProgress": "p",
    "getStckLnbInvpnDetail": "i",
}
CORE_COLUMNS = [
...
def _normalize_lending(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    rename = {
        ...
        "itmsNm": "name",
        "stckItmsNm": "name",
        # Current GetCMStckLnbInfoService response names.
        "cclStckCnt": "lending_contract_shares",
        "rdptStckCnt": "lending_repayment_shares",
        "balnStckCnt": "lending_balance_shares",
        "balnStckAmt": "lending_balance_value",
        ...
        "invpnClsfNm": "participant_type",
        "invstNm": "participant_name",
        "invpnNm": "participant_name",
        "invpnClsfDtlNm": "participant_name",
    }
    ...
    for ticker in baskets["ticker"]:
        for left, right in date_chunks(start, end, months=chunk_months):
            ...
            for endpoint in ENDPOINTS:
                # The full project path is close to Windows' legacy MAX_PATH.
                # Short partition tokens keep the atomic ".tmp" write below
                # that limit without changing source_endpoint in the data.
                endpoint_token = ENDPOINT_PATH_TOKENS[endpoint]
                path = source_root / f"t={ticker}" / endpoint_token / f"{chunk}.parquet"
                started = now_iso()
