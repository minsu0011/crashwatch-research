...
def paired_delta_table(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    base = baseline[key + cols].drop_duplicates(key, keep="last").rename(
        columns={c: f"baseline_{c}" for c in cols}
    )
    compared = work.loc[~work["experiment"].eq("baseline")].merge(
        base, on=key, how="left", validate="many_to_one"
    )
    missing_baseline = compared[[f"baseline_{c}" for c in cols]].isna().all(axis=1)
    ...
    for keys, block in deltas.groupby(group_cols, dropna=False, sort=False):
        ...
        for col in value_cols:
            ...
            if len(values):
                ...
                row[f"{col}_p_two_sided"] = float(min(1.0, 2 * min(np.mean(samples <= 0), np.mean(samples >= 0))))
            else:
