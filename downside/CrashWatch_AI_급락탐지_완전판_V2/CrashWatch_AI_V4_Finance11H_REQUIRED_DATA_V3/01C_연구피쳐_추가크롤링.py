...
def main() -> None:
    ...
    result = {
        ...
        "fred_credit_rows": len(collect_fred_credit(paths, args.start, args.end, overwrite=args.overwrite)),
        "etf_pressure_rows": len(collect_etf_pressure(paths, args.start, args.end, overwrite=args.overwrite)),
        "derivatives_rows": len(import_derivatives_csv(paths, overwrite=args.overwrite)),
    }
    if not args.skip_attention:
        result["naver_attention_rows"] = len(collect_naver_attention(
    ...
    if not args.skip_dart_enhanced:
        result["dart_ownership_rows"] = len(collect_ownership_events(
            paths, baskets, overwrite=args.overwrite, sleep_seconds=args.sleep
        ))
        result["dart_financial_rows"] = len(collect_financial_quality(
            paths, baskets, args.start, args.end, overwrite=args.overwrite, sleep_seconds=args.sleep
        ))
    else:
