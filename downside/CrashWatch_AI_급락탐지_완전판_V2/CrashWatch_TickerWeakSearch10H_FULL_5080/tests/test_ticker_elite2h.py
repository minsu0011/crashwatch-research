...
def test_adaptive_folds_keep_full_label_horizon_purges():
    p = plan()
    assert int(p["purge_days"]) >= 20
    assert int(p["calibration_purge_days"]) >= 20
    assert int(p["inner_purge_days"]) >= 20
    dates = pd.bdate_range("2016-01-01", periods=2400).to_numpy(dtype="datetime64[ns]")
    y = np.zeros(len(dates), dtype=np.int8)
    y[::11] = 1
    folds, _ = choose_adaptive_folds(dates, y, p)
    assert folds
    date_position = {value: index for index, value in enumerate(dates)}
    for fold in folds:
        fit_end = date_position[dates[fold.fit_idx].max()]
        calibration_start = date_position[dates[fold.calibration_idx].min()]
        calibration_end = date_position[dates[fold.calibration_idx].max()]
        validation_start = date_position[dates[fold.validation_idx].min()]
        inner_fit_end = date_position[dates[fold.inner_fit_idx].max()]
        inner_validation_start = date_position[dates[fold.inner_validation_idx].min()]
        assert calibration_start - fit_end - 1 >= 20
        assert validation_start - calibration_end - 1 >= 20
        assert inner_validation_start - inner_fit_end - 1 >= 20
...
def test_runtime_profile_is_full_weak_search(monkeypatch):
    from dual_ablation.ticker_elite2h.runner import load_plan
    project = Path(__file__).resolve().parents[1]
    monkeypatch.setenv("CRASHWATCH_EXECUTION_PROFILE", "full")
    full = load_plan(project)
    assert full["backend_mode"] == "hybrid_fixed"
    assert full["result_subdir"] == "ticker_weaksearch10h_full"
    assert os.environ["CUDA_VISIBLE_DEVICES"] == "0"
    assert int(full["gpu_workers"]) == 4
    assert int(full["threads_per_model_worker"]) == 4
    assert float(full["soft_budget_hours"]) == 10.0
    assert float(full["hard_cap_hours"]) >= 10.0


def test_constant_calibration_diagnostics_do_not_emit_spearman_warning():
    from dual_ablation.refine12h.calibration import _candidate_diagnostics

    y = np.asarray([0, 1] * 20, dtype=np.int8)
    raw = np.full(len(y), 0.25, dtype=float)
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        result = _candidate_diagnostics(y, raw, raw.copy())
...
def test_plan_has_soft_10h_budget_and_strong_lock(monkeypatch):
    from dual_ablation.ticker_elite2h.weak_search import load_strength_map, plan_for_tier
    project = Path(__file__).resolve().parents[1]
    monkeypatch.setenv("CRASHWATCH_EXECUTION_PROFILE", "full")
    p = load_plan(project)
    assert p["time_budget_enabled"] is True
    assert float(p["soft_budget_hours"]) == 10.0
    assert float(p["hard_cap_hours"]) == 12.0
    strength = load_strength_map(project)
    assert int((strength["search_tier"] == "strong_lock").sum()) == 5
    assert not strength.loc[strength["search_tier"].eq("strong_lock"), "training_enabled"].any()
    broad = plan_for_tier(p, "weak_priority")
    focused = plan_for_tier(p, "middle_search")
    assert len(broad["model_candidates"]) > len(focused["model_candidates"])
    assert len(broad["feature_budgets"]) > len(focused["feature_budgets"])
...
def test_registry_recovers_one_dead_worker_and_honors_retry_limit(tmp_path: Path):
    from dual_ablation.ticker_map1h.registry import TickerTaskRegistry

    registry = TickerTaskRegistry(tmp_path / "tasks.sqlite")
    registry.ensure_tasks([{
        "task_id": "elite__035420", "stage": "elite_model", "ticker": "035420",
        "variant": "weak", "seed": -1, "priority": 10, "payload": {},
    }])
    first = registry.claim_next(stages=("elite_model",), worker_name="model-2", backend="gpu", threads=3)
    assert first is not None and first.attempts == 1
    recovered = registry.recover_dead_worker("model-2", 2, "fatal cuda oom")
    assert recovered == {"released": 1, "failed": 0}
    second = registry.claim_next(stages=("elite_model",), worker_name="model-2", backend="gpu", threads=3)
    assert second is not None and second.attempts == 2
    exhausted = registry.recover_dead_worker("model-2", 2, "fatal cuda oom again")
    assert exhausted == {"released": 0, "failed": 1}
    assert registry.status()["tasks_failed"] == 1



def test_rolling_platt_writes_detailed_audit(tmp_path: Path):
    from dual_ablation.ticker_elite2h.model import _rolling_platt_policy

    rng = np.random.default_rng(44)
    dates = np.arange("2024-01-01", "2025-01-01", dtype="datetime64[D]")
    latent = rng.normal(size=len(dates))
    raw = 1.0 / (1.0 + np.exp(-(0.8 * latent - 0.4)))
    y = (latent + rng.normal(scale=0.7, size=len(dates)) > 0.4).astype(np.int8)
    audit_path = tmp_path / "audit.csv"
    policy = _rolling_platt_policy(
        y, raw, dates, plan(), audit_path=audit_path,
        context={"ticker": "005930", "fold": 0, "scope": "test"},
    )
    assert policy.method in {"platt_positive", "none"}
    assert audit_path.exists()
    audit = pd.read_csv(audit_path)
    assert {"stage", "accepted", "rejection_reason", "window_days"}.issubset(audit.columns)
    assert (audit["stage"] == "rolling_block").any()
    assert (audit["stage"] == "window_summary").any()
    assert (audit["stage"] == "final_policy").any()


def test_replay_ranking_prefers_worst_fold_and_low_variance():
    from dual_ablation.ticker_elite2h.model import _rank_replayed_recipes

    rows = []
    # Recipe A has a slightly better mean but collapses in one fold.
    for fold, pr, roc in [(0, 0.75, 0.75), (1, 0.74, 0.74), (2, 0.18, 0.51), (3, 0.73, 0.73)]:
        rows.append({
            "recipe_id": "a", "fold": fold, "family": "xgboost", "config_name": "xgb_depth3",
            "feature_policy": "standard", "feature_budget": 56, "training_window_days": 0,
            "raw_pr_auc": pr, "raw_roc_auc": roc, "raw_pr_lift": pr / 0.25,
            "balanced_accuracy": 0.60, "top_10pct_precision": 0.60, "brier_skill": -0.1,
            "positive_rate": 0.25, "calibration_method": "none",
        })
    # Recipe B has a lower peak but is stable and has a much better worst fold.
    for fold, pr, roc in [(0, 0.60, 0.68), (1, 0.59, 0.67), (2, 0.57, 0.65), (3, 0.61, 0.69)]:
        rows.append({
            "recipe_id": "b", "fold": fold, "family": "lightgbm", "config_name": "lgbm_leaf31",
            "feature_policy": "consensus", "feature_budget": 56, "training_window_days": 750,
            "raw_pr_auc": pr, "raw_roc_auc": roc, "raw_pr_lift": pr / 0.25,
            "balanced_accuracy": 0.60, "top_10pct_precision": 0.60, "brier_skill": -0.1,
            "positive_rate": 0.25, "calibration_method": "none",
        })
    ranked, common = _rank_replayed_recipes(pd.DataFrame(rows), plan())
    assert common == [0, 1, 2, 3]
    assert ranked.iloc[0]["recipe_id"] == "b"
    assert ranked.iloc[0]["selection_rule"] == "common_outer_replay_v3_worst_fold_and_variance_first"


def test_fingerprint_rebuild_uses_final_model_and_excludes_incomplete(tmp_path: Path):
    from dual_ablation.ticker_elite2h.reporting import build_fingerprints

    result = tmp_path / "result"
    model_dir = result / "ticker_models" / "005930" / "development_model"
    model_dir.mkdir(parents=True)
    (model_dir / "model_card.json").write_text(json.dumps({
        "ensemble_members": [{"family": "xgboost", "feature_policy": "consensus", "config_name": "xgb_depth3"}]
    }), encoding="utf-8")
    pd.DataFrame({"feature": ["t_finshort_value_sum_5", "u_credit_us_high_yield_oas"]}).to_csv(
        model_dir / "member_0_features.csv", index=False
    )
    summaries = pd.DataFrame([
        {"ticker": "005930", "status": "completed", "raw_pr_auc": 0.4, "raw_pr_auc_std": 0.1, "raw_pr_auc_min": 0.25,
         "raw_pr_lift": 1.5, "raw_roc_auc": 0.62, "balanced_accuracy": 0.56, "brier_skill": -0.2,
         "fold_positive_rate_std": 0.05},
        {"ticker": "000660", "status": "completed", "raw_pr_auc": 0.3, "raw_pr_auc_std": 0.2, "raw_pr_auc_min": 0.1,
         "raw_pr_lift": 1.2, "raw_roc_auc": 0.55, "balanced_accuracy": 0.52, "brier_skill": -0.3,
         "fold_positive_rate_std": 0.08},
    ])
    correlations = pd.DataFrame([{"ticker": "005930", "top40_mean_psi": 0.3}, {"ticker": "000660", "top40_mean_psi": 0.4}])
    fingerprints, _ = build_fingerprints(result, summaries, correlations)
    samsung = fingerprints.loc[fingerprints["ticker"] == "005930"].iloc[0]
    hynix = fingerprints.loc[fingerprints["ticker"] == "000660"].iloc[0]
    assert bool(samsung["fingerprint_complete"])
    assert int(samsung["preliminary_cluster"]) >= -1
    assert not bool(hynix["fingerprint_complete"])
    assert int(hynix["preliminary_cluster"]) == -1
    assert (result / "ticker_fingerprint_incomplete.csv").exists()
