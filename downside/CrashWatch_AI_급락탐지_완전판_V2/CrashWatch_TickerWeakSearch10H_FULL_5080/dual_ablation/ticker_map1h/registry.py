...
class TickerTaskRegistry:
    ...
    def recover_dead_worker(self, worker_name: str, max_attempts: int, error: str) -> dict[str, int]:
        """Return a dead worker's claim to the queue, or fail it after the retry limit."""
        released = 0
        failed = 0
        now = time.time()
        with self.connect() as conn:
            ...
            try:
                rows = conn.execute(
                    "SELECT task_id,attempts FROM tasks WHERE status='running' AND claimed_worker=?",
                    (worker_name,),
                ).fetchall()
                for row in rows:
                    ...
                    if attempts >= int(max_attempts):
                        conn.execute(
                            ...
                            "heartbeat_at=NULL,error=? WHERE task_id=? AND status='running'",
