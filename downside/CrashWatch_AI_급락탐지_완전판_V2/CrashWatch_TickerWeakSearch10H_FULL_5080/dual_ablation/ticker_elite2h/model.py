...
def feature_audit(X: np.ndarray, y: np.ndarray, feature_names: list[str], segments: int) -> pd.DataFrame:
    ...
    robust_consensus = (
        ...
        + 0.25 * _rank01(np.abs(recent_effect))
        + 0.30 * sign_agreement
        + 0.15 * (1.0 - _rank01(drift + np.nan_to_num(temporal_std)))
    )
    recent_stable = (
        0.45 * _rank01(np.abs(recent_effect))
        + 0.25 * sign_agreement
        + 0.20 * _rank01(base)
        + 0.10 * (1.0 - _rank01(drift))
    )
    return pd.DataFrame({
        ...
        "robust_consensus_score": robust_consensus,
        "recent_stable_score": recent_stable,
    })
...
def _select_recipe_for_fold(
    ...
    top_policy_count = max(1, int(plan.get("proxy_top_policies", 2)))
    top_policies = policy_frame.head(top_policy_count)[["policy", "budget"]].to_dict("records")
    ...
    scan_limit = max(2, int(plan.get("ensemble_candidate_scan", 6)))
    for _, row in model_frame.iloc[1:scan_limit].iterrows():
...
def _build_final_development_model(
    ...
    # Final inference invariant: average seed/member raw probabilities first, then apply one
    # calibrator and one monotonic prior shift. Reject any implementation/path that changes
    # ranking beyond the same safety limits used in outer folds.
    raw_roc_guard = _safe_auc(roc_auc_score, data.y[cal_idx], raw_cal)
    cal_roc_guard = _safe_auc(roc_auc_score, data.y[cal_idx], final_cal)
    raw_pr_guard = _safe_auc(average_precision_score, data.y[cal_idx], raw_cal)
    cal_pr_guard = _safe_auc(average_precision_score, data.y[cal_idx], final_cal)
    rank_guard = _safe_spearman(raw_cal, final_cal)
    guard_failed = (
        (np.isfinite(raw_roc_guard) and np.isfinite(cal_roc_guard) and cal_roc_guard < raw_roc_guard - float(plan.get("final_calibration_max_roc_drop", 0.003)))
        or (np.isfinite(raw_pr_guard) and np.isfinite(cal_pr_guard) and cal_pr_guard < raw_pr_guard - float(plan.get("final_calibration_max_pr_drop", 0.007)))
        or (not np.isfinite(rank_guard))
        or (float(rank_guard) < float(plan.get("final_calibration_min_rank_correlation", 0.997)))
    )
    if bool(plan.get("final_calibration_rank_guard", True)) and guard_failed:
        calibration_policy = SafeCalibrationPolicy(
            method="none", params={"fallback_reason": "final_post_ensemble_rank_guard", "original_policy": calibration_policy.payload()},
            threshold=choose_threshold(data.y[cal_idx], raw_cal), selection_score=math.inf,
            validation_brier=float(brier_score_loss(data.y[cal_idx], raw_cal)),
            validation_logloss=float(log_loss(data.y[cal_idx], raw_cal, labels=[0, 1])),
            validation_raw_roc_auc=raw_roc_guard, validation_calibrated_roc_auc=raw_roc_guard,
            validation_raw_pr_auc=raw_pr_guard, validation_calibrated_pr_auc=raw_pr_guard,
            rank_correlation=1.0, validation_rows=int(len(cal_idx)),
            validation_positives=int(data.y[cal_idx].sum()), fallback_reason="final_post_ensemble_rank_guard",
        )
        base_cal = raw_cal.copy()
        final_cal, prior_info = _prior_adjustment(
            data.y[fit_idx_base], data.y[cal_idx], data.dates[cal_idx], base_cal, base_cal, plan
        )
        second_rank = _safe_spearman(raw_cal, final_cal)
        if not np.isfinite(second_rank) or float(second_rank) < float(plan.get("final_calibration_min_rank_correlation", 0.997)):
            final_cal = raw_cal.copy()
            prior_info = {"used": False, "reason": "prior_adjustment_rank_guard"}
