...
def pack_results(result_dir: Path, max_mb: int = 28) -> Path:
    ...
    if registry_path.exists():
        try:
            ...
            pass
    destination = desktop_path() / f"CrashWatch_TickerWeakSearch10H_FULL_RESULTS_{pd.Timestamp.now():%Y%m%d_%H%M%S}.zip"
    core_names = [
        "RESULT_BRIEF.md", "WEAK_SEARCH_BRIEF.md", "run_summary.json", "ticker_data_readiness.csv", "ticker_elite_summary.csv",
        ...
        "ticker_fingerprint_incomplete.csv", "ticker_strength_map.csv",
        "strong_locked_manifest.csv", "weak_search_improvement.csv",
        "new_strong_candidates.csv", "soft_budget_reached.json",
    ]
    ...
    for path in sorted((result_dir / "correlation_maps").glob("*/cluster_summary.csv")):
        files.append((path, str(path.relative_to(result_dir))))
    previous_ref = result_dir / "previous_stability_reference"
    if previous_ref.exists():
        for path in sorted(previous_ref.glob("*")):
            if path.is_file():
                files.append((path, str(path.relative_to(result_dir))))
