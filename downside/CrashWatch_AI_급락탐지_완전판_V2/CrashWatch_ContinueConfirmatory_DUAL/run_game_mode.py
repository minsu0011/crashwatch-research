...
def main() -> int:
    parser=argparse.ArgumentParser(description="CrashWatch 배그 병행 저부하 누락+확증 실험")
    ...
    cfg=json.load(open(root/"config_game_mode.json",encoding="utf-8"))
    project=Path(args.project_root).expanduser().resolve() if args.project_root else root.parent.resolve()
    setup_logging(root/"launcher_game_mode.log")
