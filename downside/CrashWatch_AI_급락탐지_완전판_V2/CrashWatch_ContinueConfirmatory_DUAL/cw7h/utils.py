...
def ensure_thread_env(threads: int) -> None:
    ...
    mode = os.environ.get("CRASHWATCH_PROCESS_MODE", "").strip().lower()
    affinity_text = os.environ.get("CRASHWATCH_CPU_AFFINITY", "").strip()
    if mode or affinity_text:
        try:
            import psutil

            process = psutil.Process(os.getpid())
            if affinity_text:
                affinity = [int(value) for value in affinity_text.split(",") if value.strip()]
                if affinity:
                    process.cpu_affinity(affinity)
            if os.name == "nt" and mode == "game":
                process.nice(psutil.BELOW_NORMAL_PRIORITY_CLASS)
            elif os.name == "nt" and mode == "full":
                process.nice(psutil.ABOVE_NORMAL_PRIORITY_CLASS)
        except Exception:
            # Resource hints must never invalidate a completed model cache or
            # make an experiment fail on a restricted host.
            pass
