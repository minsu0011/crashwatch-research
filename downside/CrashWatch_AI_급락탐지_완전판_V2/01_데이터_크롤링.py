...
# NumPy 2.5와 yfinance 1.x 조합에서 발생하는 외부 DeprecationWarning이
# 장시간 수집 로그를 수십만 줄로 늘리지 않도록 숨긴다.
...
from pykrx import stock  # noqa: E402  # .env의 KRX_ID/KRX_PW를 먼저 읽은 뒤 import
DATA_ROOT = BASE_DIR / "crashwatch_ai_data"
START_DATE = os.getenv("CRASHWATCH_START_DATE", "2018-01-01")
END_DATE = os.getenv(
    "CRASHWATCH_END_DATE",
...
TOP_N_PER_MARKET_PER_MONTH = int(os.getenv("CRASHWATCH_TOP_N_PER_MARKET_MONTH", "350"))
MAX_UNIVERSE = int(os.getenv("CRASHWATCH_MAX_UNIVERSE", "1200"))
MARKETS = ("KOSPI", "KOSDAQ")
REQUEST_SLEEP_SECONDS = float(os.getenv("CRASHWATCH_REQUEST_SLEEP_SECONDS", "0.45"))
MAX_RETRIES = int(os.getenv("CRASHWATCH_MAX_RETRIES", "6"))
OVERWRITE = os.getenv("CRASHWATCH_OVERWRITE", "0").strip().lower() in {"1", "true", "yes"}
DOWNLOAD_RISK_DOCUMENTS = os.getenv("DOWNLOAD_RISK_DOCUMENTS", "1").strip().lower() in {
    "1", "true", "yes"
}
MAX_RISK_DOCUMENTS = int(os.getenv("MAX_RISK_DOCUMENTS", "6000"))
KRX_AVAILABLE = os.getenv("CRASHWATCH_FORCE_FALLBACK", "0").strip().lower() not in {"1", "true", "yes"}
...
GDELT_MAX_QUERIES = int(os.getenv("CRASHWATCH_GDELT_MAX_QUERIES", "8"))
GDELT_REQUEST_SLEEP_SECONDS = float(
    os.getenv("CRASHWATCH_GDELT_REQUEST_SLEEP_SECONDS", "15")
)
GDELT_MAX_RETRIES = int(os.getenv("CRASHWATCH_GDELT_MAX_RETRIES", "4"))
...
# pykrx 일부 버전이 logging.info(args, kwargs)를 잘못 호출해 Python 3.13에서
# 로그 출력 자체가 예외를 대량 발생시키는 것을 억제한다. 수집 오류는 collector로 기록한다.
...
def gdelt_get_json(url: str, params: dict[str, Any]) -> dict[str, Any]:
    """GDELT 전용 요청기: 429 Retry-After와 긴 백오프를 존중한다."""
    last: Exception | None = None
    for attempt in range(1, GDELT_MAX_RETRIES + 1):
        if attempt > 1:
            time.sleep(min(180.0, GDELT_REQUEST_SLEEP_SECONDS * (attempt - 1)))
        try:
            response = SESSION.get(url, params=params, timeout=(15, 120))
            if response.status_code == 429:
                ...
                try:
                    ...
                    server_delay = 0.0
                delay = min(180.0, max(server_delay, GDELT_REQUEST_SLEEP_SECONDS * attempt))
                last = RuntimeError(f"GDELT HTTP 429 (재시도 대기 {delay:.0f}초)")
                LOGGER.warning("GDELT 429 %d/%d: %.0f초 후 재시도", attempt, GDELT_MAX_RETRIES, delay)
            ...
            if attempt >= GDELT_MAX_RETRIES:
                break
            if not isinstance(exc, RuntimeError) or "HTTP 429" not in str(exc):
                time.sleep(min(180.0, GDELT_REQUEST_SLEEP_SECONDS * attempt))
    assert last is not None
    raise last
...
def krx_business_days() -> pd.DatetimeIndex:
    global KRX_AVAILABLE
    if not OVERWRITE and (META_DIR / "krx_unavailable.flag").exists():
        KRX_AVAILABLE = False
    ...
    try:
        ...
        KRX_AVAILABLE = False
        (META_DIR / "krx_unavailable.flag").write_text(
            "KRX endpoint returned an invalid response; use Yahoo fallback until explicitly retried.",
            encoding="utf-8",
        )
...
def build_historical_universe() -> pd.DataFrame:
    ...
    if path.exists() and not OVERWRITE and KRX_AVAILABLE:
        return pd.read_parquet(path)
    if not KRX_AVAILABLE:
        LOGGER.warning("KRX 응답이 차단되어 강제 포함 종목 + Yahoo Finance 대체 경로를 사용합니다.")
            ...
            [
                {
                    ...
                    "first_snapshot": pd.Timestamp(START_DATE),
                    "last_snapshot": pd.Timestamp(END_DATE),
                    "name": name,
                    "forced": True,
                }
...
def collect_one_krx_ticker(row: pd.Series) -> dict[str, Any]:
    ...
    if not KRX_AVAILABLE:
        return collect_one_yfinance_kr_ticker(row, out_path)
    ...
    jobs: list[tuple[str, Callable[[], pd.DataFrame], str]] = [
        ...
        (
            "fundamental",
            lambda: stock.get_market_fundamental_by_date(start8, end8, ticker),
            "",
        ),
        (
            "investor_value",
...
def collect_one_yfinance_kr_ticker(row: pd.Series, out_path: Path) -> dict[str, Any]:
    """KRX가 비정상 응답을 반환할 때 Yahoo Finance의 .KS/.KQ로 OHLCV를 대체한다."""
    ticker = str(row["ticker"]).zfill(6)
    suffix = ".KQ" if str(row.get("market", "")).upper() == "KOSDAQ" else ".KS"
    ...
    try:
        ...
        if data is None or data.empty:
            raise RuntimeError(f"Yahoo Finance 데이터 없음: {symbol}")
        if isinstance(data.columns, pd.MultiIndex):
            level0 = set(data.columns.get_level_values(0))
            data = data.xs(symbol, axis=1, level=1 if "Close" in level0 else 0)
        block = data.copy()
        ...
        block["ticker"] = ticker
        block["name"] = str(row.get("name", ""))
        block["market"] = str(row.get("market", ""))
        block = block.dropna(subset=["date"]).sort_values("date").drop_duplicates(["date", "ticker"])
        atomic_parquet(block.reset_index(drop=True), out_path)
        return {"ticker": ticker, "status": "ok_yfinance", "rows": len(block), "path": str(out_path)}
    except Exception as exc:  # noqa: BLE001
        LOGGER.warning("Yahoo Finance 대체 수집 실패 %s: %s", symbol, exc)
        return {"ticker": ticker, "status": "failed", "errors": repr(exc)}
...
def collect_krx_indices() -> None:
    if not KRX_AVAILABLE:
        LOGGER.warning("KRX 지수 수집을 건너뜁니다. 교차자산 Yahoo Finance 수집은 계속합니다.")
        return
...
def collect_gdelt_news_timeline(queries: pd.DataFrame) -> None:
    ...
    for row in tqdm(work.itertuples(index=False), total=len(work), desc="GDELT 뉴스량"):
        ...
        try:
            payload = gdelt_get_json(endpoint, params=params)
...
def collect_fred() -> None:
    ...
    for series_id, alias in tqdm(FRED_SERIES.items(), desc="FRED"):
        params = {
            ...
            "sort_order": "asc",
            # 장기 구간의 vintage(output_type=4)는 FRED의 2,000일 제한을 넘는다.
            # 기본 최신 확정치 형식은 전체 학습 기간을 안정적으로 제공한다.
            "output_type": 1,
        }
        try:
...
def collect_dart_disclosures(corps: pd.DataFrame, universe: pd.DataFrame) -> pd.DataFrame:
    path = RAW_DIR / "dart" / "disclosures.parquet"
    if path.exists() and not OVERWRITE:
        return pd.read_parquet(path)

    allowed = set(universe["ticker"].astype(str).str.zfill(6))
    work = corps[corps["stock_code"].astype(str).str.zfill(6).isin(allowed)].drop_duplicates("corp_code")
    max_pages = int(os.getenv("DART_MAX_DISCLOSURE_PAGES_PER_COMPANY", "0"))
    rows = []
    for row in tqdm(work.itertuples(index=False), total=len(work), desc="DART 대상종목 공시"):
        page = 1
        while True:
            try:
                payload = dart_request(
                    "list.json",
                    {
                        ...
                        "bgn_de": date8(START_DATE),
                        "end_de": date8(END_DATE),
                        "page_no": page,
                        "page_count": 100,
                        "sort": "date",
                        "sort_mth": "desc",
                ...
                LOGGER.warning("대상종목 공시 실패 %s: %s", row.stock_code, exc)
                break
            items = payload.get("list", []) or []
            rows.extend(items)
            total_page = int(payload.get("total_page", 0) or 0)
            if not items or page >= total_page or (max_pages > 0 and page >= max_pages):
                break
            page += 1
    out = pd.DataFrame(rows)
    if not out.empty:
    ...
    write_json(
        {
            "scope": "selected_universe_per_company",
            "companies": int(len(work)),
            "start_date": START_DATE,
            "end_date": END_DATE,
            "sort": "date_desc",
            "max_pages_per_company": max_pages or "all",
        },
        META_DIR / "dart_disclosure_collection.json",
    )
    return out
...
def collect_dart_risk_documents(disclosures: pd.DataFrame) -> None:
    ...
    for row in tqdm(work.itertuples(index=False), total=len(work), desc="DART 위험공시 원문"):
        ...
        if rcept_no in done and zip_path.exists() and text_path.exists() and not OVERWRITE:
            continue
        try:
...
def collect_dart(universe: pd.DataFrame) -> None:
    ...
    disclosures = collect_dart_disclosures(corps, universe)
