    ...
    if download_documents and not history_all.empty:
        ...
        for report in regular.itertuples(index=False):
            ...
            if path.exists() and not overwrite:
                ...
                try:
                    retry_call(lambda: _download_document(session, key, rcept_no, path), attempts=4, base_delay=2.0)
    ...
    for row in ticker_corp.itertuples(index=False):
        ...
        for year in range(start_year, end_year + 1):
            for report_type, report_code in REPORT_CODES.items():
                path = root / "financial_latest_conservative_pit" / f"ticker={ticker}" / f"year={year}" / f"{report_type}.parquet"
                started = now_iso()
                timer = time.perf_counter()
                if path.exists() and not overwrite:
