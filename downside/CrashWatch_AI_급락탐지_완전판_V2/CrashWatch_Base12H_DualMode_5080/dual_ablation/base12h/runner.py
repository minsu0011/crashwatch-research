...
MAX_CONSECUTIVE_BLOCK_FAILURES = 3
...
def _select_affinity(
    eligible_cpus: list[int],
    profile: str,
    threads: int,
    worker_index: int,
    worker_count: int,
) -> list[int]:
    """Select CPUs from the worker's original eligible set.

    This must be a pure selection over the original set.  Re-selecting from the
    process's already restricted affinity shrinks PUBG workers from 4 CPUs to
    2, then 1, whenever a new block is claimed.
    """
    available = sorted(set(int(cpu) for cpu in eligible_cpus))
    if not available:
        return []
    worker_count = max(1, int(worker_count))
    worker_index = min(max(0, int(worker_index)), worker_count - 1)
    threads = max(1, int(threads))
    if profile == "pubg":
        physical_first = available[::2] or available
        pools = np.array_split(np.asarray(physical_first, dtype=int), worker_count)
        pool = pools[worker_index].tolist() or physical_first
        selected = pool[:threads]
        if len(selected) < threads:
            selected.extend(cpu for cpu in available if cpu not in selected)
        return selected[:threads]
    chunks = np.array_split(np.asarray(available, dtype=int), worker_count)
    pool = chunks[worker_index].tolist() or available
    return pool[:threads]
...
def _apply_affinity(
    ...
    worker_count: int,
    *,
    eligible_cpus: list[int] | None = None,
) -> dict[str, Any]:
    import psutil
    ...
    available = (
        list(eligible_cpus)
        if eligible_cpus is not None
        else process.cpu_affinity()
        if hasattr(process, "cpu_affinity")
        else list(range(psutil.cpu_count() or threads))
    )
    selected = _select_affinity(available, profile, threads, worker_index, worker_count)
    if profile == "pubg":
        if hasattr(process, "cpu_affinity"):
    ...
    return {
        "affinity": selected,
        "eligible_cpus": sorted(set(int(cpu) for cpu in available)),
        "profile": profile,
        "threads": threads,
    }
...
class Base12HWorker:
    ...
    def load(self) -> None:
        ...
        high_missing = set(catalog.get("u_market_high_missing", []))
        market_low_missing = sorted(set(market_cols) - high_missing)
        rates_conditions = sorted(set(catalog.get("u_market_rates", [])) | set(catalog.get("u_market_conditions", [])))
        catalog["u_market_low_missing"] = market_low_missing
        catalog["u_market_rates_conditions"] = rates_conditions
        catalog["u_core_short_rates_conditions"] = sorted(
            set(catalog.get("u_financial_shorting", [])) | set(rates_conditions)
        )
        catalog["u_core_short_market_low_missing"] = sorted(
            set(catalog.get("u_financial_shorting", [])) | set(market_low_missing)
        )
        catalog["u_core_all_low_missing"] = sorted(
            set(catalog.get("u_financial_shorting", []))
            | set(market_low_missing)
            | set(catalog.get("u_etf_pressure", []))
        )
...
def _worker_main(payload: dict[str, Any]) -> None:
    ...
    initial_affinity = _apply_affinity(profile, default_threads, worker_index, worker_count)
    eligible_cpus = initial_affinity["eligible_cpus"]
    if profile == "pubg":
    ...
    while not worker.stop_requested():
        ...
        _apply_affinity(
            "pubg" if claim.backend == "cpu" else "full",
            claim.threads,
            worker_index,
            worker_count,
        ...
        try:
            ...
            if consecutive_failures >= MAX_CONSECUTIVE_BLOCK_FAILURES:
                raise RuntimeError(
                    f"worker aborted after {consecutive_failures} consecutive block failures; "
                    "supervisor/watchdog must inspect and resume"
                )
            time.sleep(3)
