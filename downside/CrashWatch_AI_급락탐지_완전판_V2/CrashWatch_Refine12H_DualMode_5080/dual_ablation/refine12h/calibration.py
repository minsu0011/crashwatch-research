...
def _candidate_diagnostics(y: np.ndarray, raw: np.ndarray, calibrated: np.ndarray) -> dict[str, float]:
    ...
    if np.nanstd(raw) < 1e-15 or np.nanstd(calibrated) < 1e-15:
        correlation = 1.0 if np.allclose(raw, calibrated) else 0.0
    else:
        correlation = spearmanr(raw, calibrated, nan_policy="omit").statistic
    return {
        "raw_roc": raw_roc,
        "cal_roc": cal_roc,
        "raw_pr": raw_pr,
