...
class FocusedNestedRunner:
    ...
    def load(self) -> None:
        ...
        missing_buckets = sorted({x["bucket"] for x in self.plan["bucket_combinations"]} - available_buckets)
        if missing_buckets:
    ...
    def _scope_metrics(self, pred: pd.DataFrame, exp: Experiment, outer_fold: int, seed: int, policy: CalibrationPolicy, task_id: str) -> pd.DataFrame:
        ...
        if exp.name == "baseline":
            for bucket in sorted({x["bucket"] for x in self.plan["bucket_combinations"]}):
    ...
    def summarize(self, metrics: pd.DataFrame) -> dict[str, Any]:
        ...
        if summary.empty:
            ...
            summary["test_family"] = np.where(summary["stage"].eq("focused_global"), "global_all", "bucket_target")
        ...
        try:
            with RunLock(self.lock_path, self.profile):
                ...
                for outer_fold in range(len(self.outer_folds)):
                    ...
                    self._state(outer_fold=outer_fold, total_planned_outer_tasks=total_planned)
                    ...
                    for exp in self.experiments():
                        ...
                        self._state(outer_fold=outer_fold, experiment=exp.name, selected_config=config.name)
                        ...
                        for seed in self.seeds:
                            ...
                            self._state(outer_fold=outer_fold, experiment=exp.name, seed=seed, selected_config=config.name, calibration=policy.method)
