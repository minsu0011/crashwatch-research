...
def classify_report(report_name: str) -> tuple[str, int]:
    ...
    # The original archive's Korean literals were mojibake.  These are the
    # actual disclosure terms returned by OpenDART.
    rules = [
    ...
    for _, row in mapping.dropna(subset=["corp_code"]).iterrows():
        ...
        while True:
            try:
                ...
                # A single throttled or unavailable company must not abort the
                # whole disclosure panel.
                LOGGER.warning("DART %s request failed: %s", ticker, exc)
                break
