...
# Some distributed V3 archives contained mojibake versions of the Korean
# PyKRX headers above.  Keep the legacy map for backward compatibility, then
# explicitly register the real headers returned by PyKRX.
COLUMN_MAP.update({
    "시가": "open", "고가": "high", "저가": "low", "종가": "close",
    "거래량": "volume", "거래대금": "trading_value", "등락률": "return_pct",
    "시가총액": "market_cap", "상장주식수": "listed_shares",
    "외국인보유수량": "foreign_owned_shares", "외국인소진율": "foreign_ownership_rate",
    "상장주식수": "listed_shares", "보유수량": "foreign_owned_shares",
    "지분율": "foreign_ownership_rate", "한도수량": "foreign_limit_shares",
    "공매도": "short_volume", "비중": "short_ratio",
    "잔고수량": "short_balance_shares", "잔고금액": "short_balance_value",
    "기관합계": "institution", "기관": "institution", "개인": "individual",
    "외국인": "foreign", "기타법인": "other_corp", "금융투자": "financial_investment",
    "보험": "insurance", "투신": "investment_trust", "사모": "private_fund",
    "은행": "bank", "기타금융": "other_finance", "연기금": "pension",
    "기타외국인": "other_foreign", "합계": "total",
})
...
# Several KRX endpoints require an interactive KRX session and return HTML
# (rather than JSON) in unattended runs.  Retrying them five times per ticker
# turns a recoverable missing optional field into hours of idle time.
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=0.5, min=0.5, max=2), reraise=True)
...
def collect_one_ticker(row: pd.Series, start: str, end: str, sleep_seconds: float = 0.35) -> tuple[pd.DataFrame, list[str]]:
    ...
    # KRX now gates these endpoints behind a logged-in session.  They emit
    # non-JSON HTML without credentials; skip them deterministically instead
    # of retrying for every basket ticker.  OHLCV remains public.
    if os.getenv("KRX_ID") and os.getenv("KRX_PW"):
        ...
        errors.append("authenticated KRX endpoints skipped: KRX_ID/KRX_PW not configured")
...
def collect_basket_tickers(paths: ProjectPaths, baskets: pd.DataFrame, start: str, end: str, *, overwrite: bool = False, sleep_seconds: float = 0.35) -> pd.DataFrame:
    ...
    for _, row in baskets.iterrows():
        ...
        existing = pd.read_parquet(path) if path.exists() and not overwrite else pd.DataFrame()
        ...
        if existing.empty:
            ...
            actual_start, actual_end = existing_dates.min(), existing_dates.max()
            if requested_start < actual_start:
                ranges.append((requested_start, actual_start - pd.Timedelta(days=1)))
            if requested_end > actual_end:
                ranges.append((actual_end + pd.Timedelta(days=1), requested_end))
        if not ranges:
            records.append({
                "ticker": ticker, "status": "cached", "rows": len(existing),
                "start": pd.to_datetime(existing["date"]).min(), "end": pd.to_datetime(existing["date"]).max(),
                "path": str(path), "attempt_count": 0,
            })
            continue
        additions, errors = [], []
        for range_start, range_end in ranges:
            if range_start > range_end:
                continue
            incoming, range_errors = collect_one_ticker(
                row, str(range_start.date()), str(range_end.date()), sleep_seconds,
            )
        ...
        if not additions and existing.empty:
            records.append({
                ...
                "errors": " | ".join(errors), "attempt_count": len(ranges),
            })
            continue
        incoming = pd.concat(additions, ignore_index=True, sort=False) if additions else pd.DataFrame()
        frame = merge_newer(existing, incoming, ["date", "ticker"])
        atomic_parquet(frame, path)
        records.append({
            "ticker": ticker, "name": row.get("name", ""), "bucket": row.get("bucket", ""),
            "status": "ok" if additions else "cached_partial", "rows": len(frame),
            "start": frame["date"].min(), "end": frame["date"].max(),
            "errors": " | ".join(errors), "path": str(path), "attempt_count": len(ranges),
        })
