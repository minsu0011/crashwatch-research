...
def _first_numeric(df: pd.DataFrame, candidates: list[str]) -> np.ndarray:
    for col in candidates:
        if col in df.columns:
            return pd.to_numeric(df[col], errors="coerce").to_numpy(dtype=np.float64, copy=True)
...
def _build_ticker_block(block: pd.DataFrame) -> pd.DataFrame:
    ...
    foreign_value = _first_numeric(block, ["ff_net_value_foreign", "ff_value_foreign", "flow_value_foreign", "t_flow_foreign_net_buy_value"])
    institution_value = _first_numeric(block, ["ff_net_value_institution", "ff_value_institution", "flow_value_institution", "t_flow_institution_net_buy_value"])
    ...
    foreign_ownership = _first_numeric(block, ["ff_foreign_ownership_rate", "ff_foreign_foreign_ownership_rate", "foreign_foreign_ownership_rate", "t_foreign_ownership_rate"])
