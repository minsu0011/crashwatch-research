    ...
    for ticker_row in baskets.itertuples(index=False):
        ...
        if ticker_root.exists():
            for source_dir in sorted(ticker_root.glob("source=*")):
                frame = read_parquet_tree(source_dir)
                if frame.empty:
                    continue
                frame = normalize_date(frame).sort_values("date").drop_duplicates("date", keep="last")
                frame = frame.drop(columns=["ticker", "bucket", "name"], errors="ignore")
