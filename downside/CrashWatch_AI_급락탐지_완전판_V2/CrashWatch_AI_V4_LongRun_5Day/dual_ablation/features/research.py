...
def _tail_dependence_block(g: pd.DataFrame, market_ret: pd.Series) -> pd.DataFrame:
    ...
    def conditional_beta(mask: pd.Series, window: int, min_observations: int) -> pd.Series:
        y = ret.where(mask)
        x = mkt.where(mask)
        covariance = y.rolling(window, min_periods=min_observations).cov(x)
        variance = x.rolling(window, min_periods=min_observations).var()
        return safe_div(covariance, variance)

    for window in (60, 120):
        minp = max(12, window // 4)
        down_mask = mkt < 0
        tail_threshold = mkt.rolling(window, min_periods=minp).quantile(0.20)
        tail_mask = mkt <= tail_threshold
        down_min = max(10, window // 6)
        tail_min = max(6, window // 10)
        out[f"t_taildep_downside_beta_{window}"] = conditional_beta(down_mask, window, down_min)
        out[f"t_taildep_tail_beta_{window}"] = conditional_beta(tail_mask, window, tail_min)
        out[f"t_taildep_down_corr_{window}"] = ret.where(down_mask).rolling(
            window, min_periods=down_min
        ).corr(mkt.where(down_mask))
        cocrash = ((ret <= -0.05) & (mkt <= -0.02)).astype(float)
        cocrash = cocrash.where(ret.notna() & mkt.notna(), np.nan)
        out[f"t_taildep_cocrash_freq_{window}"] = cocrash.rolling(window, min_periods=minp).mean()
...
def _merge_optional_ticker_sources(base: pd.DataFrame, paths: ProjectPaths) -> pd.DataFrame:
    ...
    if not ownership.empty:
        ...
        for ticker, g in out.groupby("ticker", sort=False):
            ...
            coverage_start = e["date"].min() if not e.empty else pd.NaT
            available = block["date"].ge(coverage_start) if pd.notna(coverage_start) else pd.Series(False, index=block.index)
            block["t_owner_source_is_available"] = available.astype("int8")
            for event_type in ("major", "insider"):
                x = e[e["event_type"].eq(event_type)][["date", "change_shares", "event_count"]]
                merged = block[["date"]].merge(x, on="date", how="left")
                availability_array = available.to_numpy()
                change = merged["change_shares"].fillna(0.0).where(availability_array, np.nan)
                event_count = merged["event_count"].fillna(0.0).where(availability_array, np.nan)
                block[f"t_owner_{event_type}_net_change_20"] = change.rolling(20, min_periods=1).sum().to_numpy()
                sell = change.lt(0).astype(float).where(availability_array, np.nan)
                buy = change.gt(0).astype(float).where(availability_array, np.nan)
                block[f"t_owner_{event_type}_sell_count_20"] = sell.rolling(20, min_periods=1).sum().to_numpy()
                block[f"t_owner_{event_type}_buy_count_20"] = buy.rolling(20, min_periods=1).sum().to_numpy()
                block[f"t_owner_{event_type}_event_count_20"] = event_count.rolling(20, min_periods=1).sum().to_numpy()
            parts.append(block)
...
def add_research_ticker_features(
    ...
    market_candidates = {
        "KOSPI": ["u_global_kospi_ret_1", "u_market_kospi_ret_1", "u_market_ret_1"],
        "KOSDAQ": ["u_global_kosdaq_ret_1", "u_market_kosdaq_ret_1", "u_market_ret_1"],
    }

    def market_frame(market_name: str) -> pd.DataFrame:
        candidates = market_candidates.get(market_name.upper(), market_candidates["KOSPI"])
        column = next((c for c in candidates if c in universe_features.columns), None)
        if column is None:
            return pd.DataFrame({"date": df["date"].unique(), "_market_ret": np.nan})
        return universe_features[["date", column]].rename(columns={column: "_market_ret"}).copy()

    pieces = []
    for _, g in df.groupby("ticker", sort=False):
        g = g.sort_values("date")
        market_name = str(g["market"].dropna().iloc[0]) if "market" in g.columns and g["market"].notna().any() else "KOSPI"
        market = market_frame(market_name)
        # merge()는 RangeIndex를 새로 만들기 때문에 원래 g 인덱스와 그대로
        # 연산하면 첫 종목 이외의 시장수익률이 인덱스 정렬 과정에서 NaN이 된다.
        # 피처 블록이 사용하는 g 인덱스로 명시적으로 되돌려 종목마다 동일한
        # point-in-time 시장수익률을 사용한다.
        market_values = g[["date"]].merge(market, on="date", how="left")["_market_ret"].to_numpy()
        m = pd.Series(market_values, index=g.index, dtype="float64")
        blocks = [
...
def _optional_universe_sources(paths: ProjectPaths) -> list[pd.DataFrame]:
    ...
    if not fred.empty:
        ...
        raw_columns = [c for c in f.columns if c != "date"]
        # 미국 장 마감/공표가 한국 당일 장 마감보다 늦을 수 있으므로 최소 한
        # 원천 관측치를 지연한다. 최신 FRED 파일의 수정 가능성은 정책표에 별도 표시한다.
        f[raw_columns] = f[raw_columns].apply(pd.to_numeric, errors="coerce").shift(1)
