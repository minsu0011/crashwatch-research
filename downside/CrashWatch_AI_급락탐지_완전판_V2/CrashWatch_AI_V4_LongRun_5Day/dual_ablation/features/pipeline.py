...
def merge_with_training(paths: ProjectPaths, universe: pd.DataFrame, ticker: pd.DataFrame, training_path: Path | None = None) -> tuple[pd.DataFrame, Path | None]:
    ...
    # V3 training_dataset_dual을 기반으로 V4 피처를 증분 생성할 때 V3 피처를
    # 다시 병합하면 수백 개의 *_dual 중복 열이 생긴다. 기존 봉인 산식은 유지하고
    # 새로 생성된 열만 더해 데이터 크기와 모델 입력의 중복을 방지한다.
    universe_add = [c for c in universe.columns if c == "date" or c not in base.columns]
    merged = base.merge(universe[universe_add], on="date", how="left", validate="many_to_one")
    ticker_keys = {"date", "ticker"}
    ticker_add = [c for c in ticker.columns if c in ticker_keys or c not in merged.columns]
    merged = merged.merge(ticker[ticker_add], on=["date", "ticker"], how="left", validate="many_to_one")
    if merged.columns.duplicated().any():
        duplicated = merged.columns[merged.columns.duplicated()].tolist()
        raise ValueError(f"피처 결합 후 중복 열이 생겼습니다: {duplicated[:20]}")
