...
def _candidate_diagnostics(y: np.ndarray, raw: np.ndarray, calibrated: np.ndarray) -> dict[str, float]:
    ...
    raw_finite = np.asarray(raw)[finite]
    calibrated_finite = np.asarray(calibrated)[finite]
    if (
        finite.sum() < 2
        or np.unique(raw_finite).size < 2
        or np.unique(calibrated_finite).size < 2
        or np.std(raw_finite) < 1e-15
        or np.std(calibrated_finite) < 1e-15
    ):
        correlation = 1.0 if len(raw_finite) and np.allclose(raw_finite, calibrated_finite) else 0.0
    else:
