...
def load_plan(project: Path) -> dict[str, Any]:
    ...
    if profile == "cpu4":
        ...
        plan.update({
            ...
            "result_subdir": "ticker_independent_cpu4_stability_fix",
            "cache_namespace": "ticker_independent_cpu4_stability_fix_v4",
            "gpu_workers": 1,
    ...
    try:
        ...
        if os.name == "nt":
            ...
            if profile == "full":
                ...
                # Keep the four pinned worker threads from competing with a foreground game.
            ...
            try:
                ...
                if task.stage == "correlation_map":
                    ...
                    if summary.get("status") == "partial":
                        registry.release(task.task_id, "manual stop partial; fold/replay checkpoints preserved")
                        return 0
...
def _resource_monitor(result_dir: Path, plan: dict[str, Any], done: threading.Event) -> None:
    ...
    cpu_only = str(plan.get("execution_profile", "cpu4")) == "cpu4"
    sealed_gpu_stats = {
        key: np.nan
        for key in ["gpu_utilization_pct", "vram_used_mb", "vram_total_mb", "gpu_temperature_c", "gpu_power_w"]
    }
    ...
    while not done.wait(float(plan["resource_poll_seconds"])):
        ...
        row = {
            ...
            # In game mode the GPU belongs to the foreground game. Do not poll it
            # or stop this CPU-only run because of the game's temperature.
            **(sealed_gpu_stats if cpu_only else _nvidia_stats()),
        }
        ...
        high_temp_count = (
            0 if cpu_only
            else high_temp_count + 1 if np.isfinite(temperature) and temperature >= float(plan["gpu_temperature_stop"])
            else 0
        )
