...
def test_runtime_profiles_have_separate_backends_and_result_dirs(monkeypatch):
    ...
    assert cpu["result_subdir"] == "ticker_independent_cpu4_stability_fix"
    assert os.environ["CUDA_VISIBLE_DEVICES"] == "-1"
    assert int(cpu["threads_per_model_worker"]) == 4
    assert int(cpu["correlation_workers"]) == 0
...
def test_constant_calibration_diagnostics_do_not_emit_spearman_warning():
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        result = _candidate_diagnostics(y, raw, calibrated)
