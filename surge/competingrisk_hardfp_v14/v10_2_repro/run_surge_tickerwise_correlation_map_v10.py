...
def compute_train_feature_scores(
    ...
    *,
    ...
    # Materialize the training slice once.  The original implementation rebuilt
    # an indexed pandas frame for every feature (439 times per ticker/fold).
    # Keeping the same column order and float conversion makes this an exact
    # execution optimization, while substantially reducing allocator pressure.
    feature_matrix = frame.iloc[indices][list(features)].to_numpy(dtype=float)
    rows: list[dict[str, Any]] = []
    for feature_position, feature in enumerate(features):
        values = feature_matrix[:, feature_position]
...
def greedy_deduplicate_features(
    ...
    *,
    ...
    candidate_matrix = frame.iloc[indices][candidates].to_numpy(dtype=float) if candidates else np.empty((len(indices), 0))
    selected: list[str] = []
    selected_arrays: list[np.ndarray] = []
    for feature_position, feature in enumerate(candidates):
        values = candidate_matrix[:, feature_position]
        keep = True
...
def build_ticker_base_oof(
    ...
    if args.resume and cache_path.exists() and manifest_path.exists():
        ticker_dtype = {args.ticker_column: str, "ticker": str}
        cached = pd.read_csv(cache_path, dtype=ticker_dtype)
        manifest = pd.read_csv(manifest_path, dtype=ticker_dtype)
    ...
    for ticker in sorted(ticker_fold_index):
        for fold in folds:
                    ...
                    records.append(
                        {
                            ...
                            "base_model_count": 0,
                            "base_backends": "",
                            "model_status": "LOW_EVIDENCE_NO_MODEL",
                        }
                    )
                continue
            ...
            def fit_backend(backend: str) -> tuple[str, np.ndarray]:
                return backend, train_backend_predict(
                    backend,
                    x_train,
                    y_train,
                    x_validation,
            ...
            if bool(getattr(args, "parallel_backends", False)) and len(backends) > 1:
                with ThreadPoolExecutor(max_workers=len(backends), thread_name_prefix="ticker-backend") as pool:
                    backend_results = list(pool.map(fit_backend, backends))
            else:
                backend_results = []
                for backend in backends:
                    try:
                        backend_results.append(fit_backend(backend))
                    except Exception as exc:
                        if args.strict_backend:
                            raise
                        log(f"Ticker base backend failed ticker={ticker} fold={fold.fold_id} backend={backend}: {exc}")
            for backend, score in backend_results:
                try:
...
def compute_ticker_stage0_maps(
    ...
    if args.resume and cache.exists():
        ...
        rows: list[dict[str, Any]] = []
        for (ticker, fold_id), part in combined.groupby([args.ticker_column, "fold_id"], sort=True):
            ...
            log(f"Ticker stage0 map ticker={ticker} fold={fold_id} features={len(features)}")
...
def compute_ticker_extended_maps(
    ...
    rows: list[dict[str, Any]] = []
    for (ticker, fold_id), part in merged.groupby([args.ticker_column, "fold_id"], sort=True):
        codes = part["error_code"].to_numpy(dtype=np.int8)
        y = part[args.target_column].to_numpy(dtype=np.int8)
        ab_pairs = ab_lookup.get((str(ticker), int(fold_id)), pd.DataFrame())
        cd_pairs = cd_lookup.get((str(ticker), int(fold_id)), pd.DataFrame())
        ...
        log(f"Ticker extended map ticker={ticker} fold={fold_id}")
    by_fold = pd.DataFrame(rows)
    if by_fold.empty:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--parallel-backends", action=argparse.BooleanOptionalAction, default=False)
